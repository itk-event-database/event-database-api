Feature: Events
  ...
  Scenario: Get future events
    When I add "Content-Type" header equal to "application/ld+json"
    And I add "Accept" header equal to "application/ld+json"
    And I send a "GET" request to "/api/events?occurrences.startDate[after]=2050-01-01"
    And the JSON node "hydra:member" should have 1 element
    And the JSON node "hydra:member[0].@id" should be equal to "/api/events/2"
  ...
  Scenario: Get past events
    When I add "Content-Type" header equal to "application/ld+json"
    And I add "Accept" header equal to "application/ld+json"
    And I send a "GET" request to "/api/events?occurrences.startDate[before]=2050-01-01&occurrences.endDate[after]=@0"
    And the JSON node "hydra:member" should have 1 element
  ...
  Scenario: Get all events
    When I add "Content-Type" header equal to "application/ld+json"
    And I add "Accept" header equal to "application/ld+json"
    And I send a "GET" request to "/api/events?occurrences.startDate[after]=1900-01-01&occurrences.endDate[after]=1900-01-01"
