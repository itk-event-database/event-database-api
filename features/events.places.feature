Feature: Events
  ...
  # @FIXME: Something is broken, but is it the test or the code?
  #
  ...
  Scenario: Create a new place
    ...
    And the header "Content-Type" should be equal to "application/ld+json; charset=utf-8"
    And the JSON node "occurrences" should have 1 element
    And the JSON node "occurrences[0].place.@id" should be equal to "/api/places/2"

  Scenario: Cannot update place name
    When I authenticate as "api-write"
    And I add "Content-Type" header equal to "application/ld+json"
    And I add "Accept" header equal to "application/ld+json"
    When I send a "POST" request to "/api/events" with body:
    ...
    And the JSON node "occurrences" should have 1 element
    And the JSON node "occurrences[0].place.@id" should be equal to "/api/places/2"
    And the JSON node "occurrences[0].place.name" should be equal to "A new place"
