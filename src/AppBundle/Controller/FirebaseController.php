...
class FirebaseController extends Controller
{
    ...
    public function eventsAction()
    {
        ...
        $sql .= ' and deleted_at is null and is_published = 1 and and has_public_access = 1';
        $stmt = $this->getDoctrine()->getManager()->getConnection()->prepare($sql);
        $stmt->execute(['now' => $this->getNow()]);
        $data = [];
        while ($row = $stmt->fetch()) {
    ...
    public function occurrencesAction()
    {
        ...
        $sql .= ' and event_id in (select id from event where deleted_at is null and is_published = 1 and has_public_access = 1)';
